...
public class ControlCenter {
    ...
    public ControlCenter(Context context) {
        ...
        mTimer = new Timer(CrazyLinkConstant.MAX_TIME);
        mAnimalPic = new int[(int) CrazyLinkConstant.GRID_NUM][(int) CrazyLinkConstant.GRID_NUM];
        mPicBak = new int[(int) CrazyLinkConstant.GRID_NUM][(int) CrazyLinkConstant.GRID_NUM];
        mEffect = new int[(int) CrazyLinkConstant.GRID_NUM][(int) CrazyLinkConstant.GRID_NUM];
        mDisappearToken = new int[(int) CrazyLinkConstant.GRID_NUM][(int) CrazyLinkConstant.GRID_NUM];
    ...
    public static void init() {
        for (int i = 0; i < (int) CrazyLinkConstant.GRID_NUM; i++) {
            for (int j = 0; j < (int) CrazyLinkConstant.GRID_NUM; j++) {
    ...
    static void fillGrid(int col, int row) {
        if (0 == mAnimalPic[col][row]) {
            for (int i = row; i < (int) CrazyLinkConstant.GRID_NUM; i++) {
    ...
    static void fillMethod() {
        for (int i = 0; i < (int) CrazyLinkConstant.GRID_NUM; i++) {
            for (int j = 0; j < (int) CrazyLinkConstant.GRID_NUM; j++) {
                ...
                {
                    if (j < (int) CrazyLinkConstant.GRID_NUM - 1) {
                        //从上一行中跌落
    ...
    void autoTip() {
        ...
        for (int i = 1; i < (int) CrazyLinkConstant.GRID_NUM - 1; i++) {
            for (int j = 1; j < (int) CrazyLinkConstant.GRID_NUM - 1; j++) {
    ...
    static void markBomb(int token, int col, int row) {
        ...
        if (isBomb(col, row)) {
            ...
            for (int i = col - 1; i <= col + 1; i++) {
                if (i >= 0 && i < (int) CrazyLinkConstant.GRID_NUM) {
                    for (int j = row - 1; j <= row + 1; j++) {
                        if (j >= 0 && j < (int) CrazyLinkConstant.GRID_NUM) {
    ...
    static void genSpecialAnimal() {
        ...
        data = (data % ((int) CrazyLinkConstant.GRID_NUM * (int) CrazyLinkConstant.GRID_NUM));
        int x = 0;
        int y = 0;
        x = data / (int) CrazyLinkConstant.GRID_NUM;
        y = data % (int) CrazyLinkConstant.GRID_NUM;
    ...
    public void initDraw(GL10 gl) {
        ...
        drawTimeBar = new DrawTimeBar(gl, timeBarTextureId, CrazyLinkConstant.MAX_TIME);
    ...
    public void run() {
        mAutoTipTimer++;
        if (mAutoTipTimer > CrazyLinkConstant.AUTOTIP_DELAY) {
            if (!mIsAutoTip) {
                mIsAutoTip = true;
                autoTip();
            }
