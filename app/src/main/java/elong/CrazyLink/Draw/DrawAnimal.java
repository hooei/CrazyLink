...
public class DrawAnimal {
    ...
    //顶点纹理数据的初始化    
    private void initTextureBuffer(int which) {
        textureRatio = 1 / 8.0f;        //图片是(7+1)个独立的素材对象组成，每次需要根据which准确地获取对应的素材
        float textureCoors[] = new float[]    //顶点纹理S、T坐标值数组
                {
                        (which - 1) * textureRatio, 0,
                        (which - 1) * textureRatio, 1,
                        which * textureRatio, 1,
                        which * textureRatio, 1,
                        which * textureRatio, 0,
                        (which - 1) * textureRatio, 0
                };
    ...
    public void draw(GL10 gl, int which, int col, int row) {
        initVertexBuffer(col, row);    //根据col,row初始化顶点坐标
        initTextureBuffer(which);    //根据witch来初始化纹理顶点数据
