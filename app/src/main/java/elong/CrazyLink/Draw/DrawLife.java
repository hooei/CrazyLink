...
public class DrawLife {
    ...
    private void initVertexBuffer(int col) {
        ...
        int w = 16 * CrazyLinkConstant.ADP_SIZE;
        int h = 16 * CrazyLinkConstant.ADP_SIZE;
    ...
    private void initNumVertexBuffer() {
        vCount = 6;//顶点的数量，一个正方形用两个三角形表示，共需要6个顶点
        float deltaX = 32 * 5 * CrazyLinkConstant.ADP_SIZE;
        float deltaY = 64 * 5 * CrazyLinkConstant.ADP_SIZE;
        int vertices[] = new int[]//顶点坐标数据数组
                {
    ...
    private void initTextureBuffer(int witch) {
        textureRatio = 1 / 2.0f;        //图片是2个独立的素材对象组成，每次需要根据witch准确地获取对应的素材
